...
- `data/festival-calendar-2026-2027.jsonl` — **4,618 festivals and events across 2,299 cities worldwide for 2026–2027**, with exact confirmed dates for 3,872 of them, a citable non-search URL for 4,173, event typing, price signals where published, recurrence rules, and per-row provenance. Facts only (no prose descriptions, no images) so the CC BY grant is clean. Generated by `scripts/export-festival-calendar.py` from the continuously curated TripSapien festival catalog.
- `data/festival-calendar-2026-2027.csv` — the same rows as a flat table.
